...
public class Enemy : EntityData
{
    ...
    private bool default_AgentState = false;
    [Space]
    ...
    [Space]
    [SerializeField] private string corpseSpawnID = null;
    ...
    protected override void OnEnable()
    {
        ...
        default_AgentState = agent.enabled;
    ...
    private void MovementFlying_Stationary()
    {
        ...
        Vector3 targetPosition = spawnPosition + (Vector3.up * hover_height);
        targetPosition += (targetPosition - transform.position) * 0.1f;
        transform.position = Vector3.Slerp(transform.position, targetPosition, Time.deltaTime * hover_speed);

        Vector3 brakingDirection = -GetRigidbody().linearVelocity;
        ApplyForce(brakingDirection, brakingDirection.magnitude * 0.25f, ForceMode.Acceleration, "Enemy.Movement.Stationary");
    }

    private void MovementFlying_Tracking()
    {
        if (GetRigidbody().constraints != RigidbodyConstraints.FreezeRotation) SetConstraints(RigidbodyConstraints.FreezeRotation);
    ...
    {
        // Check if the agent has control
        if (controlScale < 1) return;
        // Check if agent is set
        if (agent == null || !agent.enabled)
            return;
        if (!agent.isOnNavMesh && NavMesh.SamplePosition(transform.position, out NavMeshHit hit, Mathf.Clamp(GetLinearVelocity().magnitude, transform.localScale.y, 1000), NavMesh.AllAreas)) agent.Warp(hit.position);
        else if (!agent.isOnNavMesh) return;
    ...
    {
        base.DisableRagdoll();
        agent.enabled = default_AgentState;
    }
    #endregion
    ...
    {
        GameObject corpseObject = ObjectPooler.Instance.Spawn(corpseSpawnID, transform.position, Vector3.zero);
        if (corpseObject == null) return;
        Corpse corpse = corpseObject.GetComponent<Corpse>();
        corpseObject.transform.rotation = transform.rotation; 
        corpse.SetEyes(eyes);
    }
    #endregion
